...
constexpr int MAX_DET_ORDER = 16;
...
struct Args {
    ...
    std::string km_evaluator = "bareiss";
...
Int det_bareiss(const Matrix& matrix) {
    ...
    if (n == 0) return 1;
    if (n > MAX_DET_ORDER) {
        std::cerr << "det_bareiss stack limit exceeded for n=" << n << "\n";
        std::exit(2);
    }
    Int a[MAX_DET_ORDER][MAX_DET_ORDER] = {};
    for (int i = 0; i < n; ++i) {
    ...
    for (int k = 0; k < n - 1; ++k) {
        ...
        if (pivot == n) return 0;
        if (pivot != k) {
            for (int column = 0; column < n; ++column) std::swap(a[k][column], a[pivot][column]);
            sign = -sign;
        }
...
bool is_row_lex_ordered(const Matrix& matrix) {
    ...
    for (int earlier = 0; earlier < size - 1; ++earlier) {
        for (int later = earlier + 1; later < size; ++later) {
            Vec earlier_prefix = partial_row(matrix, earlier, earlier + 1);
            Vec later_prefix = partial_row(matrix, later, earlier + 1);
            if (compare_vectors_abs(earlier_prefix, later_prefix) <= 0) return false;
        }
    }
    return true;
}
...
Matrix remove_row_column(const Matrix& matrix, int removed_row, int removed_column) {
    int size = static_cast<int>(matrix.size());
    Matrix out;
    out.reserve(size - 1);
    for (int row = 0; row < size; ++row) {
        ...
        Vec out_row;
        out_row.reserve(size - 1);
        for (int column = 0; column < size; ++column) {
            if (column == removed_column) continue;
            out_row.push_back(matrix[row][column]);
        }
...
IntMatrix adjugate_by_cofactors(const Matrix& matrix) {
    int size = static_cast<int>(matrix.size());
    IntMatrix adjugate(size, std::vector<Int>(size, 0));
    if (size == 1) {
        adjugate[0][0] = 1;
        return adjugate;
    }
    for (int row = 0; row < size; ++row) {
        for (int column = 0; column < size; ++column) {
            Matrix minor = remove_row_column(matrix, row, column);
            Int cofactor = det_bareiss(minor);
            if ((row + column) % 2 != 0) cofactor = -cofactor;
...
Int det_augmented_from_adjugate(const Vec& gamma, Int det_matrix, const IntMatrix& adjugate, int c) {
    int size = static_cast<int>(gamma.size());
    Int quadratic = 0;
    for (int row = 0; row < size; ++row) {
        for (int column = 0; column < size; ++column) {
            quadratic += static_cast<Int>(gamma[row]) * adjugate[row][column] * static_cast<Int>(gamma[column]);
        }
    }
    return static_cast<Int>(c) * det_matrix - quadratic;
}

Int det_extension_from_adjugate(const Vec& vector, Int det_matrix, const IntMatrix& adjugate, int diagonal) {
    return det_augmented_from_adjugate(vector, det_matrix, adjugate, diagonal);
}
...
bool blocks_descending(const Matrix& matrix, const Blocks& blocks) {
    for (size_t index = 1; index < blocks.intervals.size(); ++index) {
        ...
        Matrix previous_block = submatrix_interval(matrix, previous.first, previous.second);
        Matrix current_block = submatrix_interval(matrix, current.first, current.second);
        if (compare_matrices_abs_lex(previous_block, current_block) < 0) return false;
    }
    return true;
}
...
int is_lexmax_block(const Matrix& block, State& state) {
    ...
    for (int first = 0; first < size; ++first) {
        for (int second = 0; second < size; ++second) {
            ...
            if (comparison >= 0) current.insert(std::move(permutation));
        }
    }
    ...
    for (int leading_size = 3; leading_size <= size; ++leading_size) {
        ...
        for (const Vec& permutation : current) {
            for (int swap_position = leading_size - 1; swap_position < size; ++swap_position) {
                ...
                if (comparison >= 0) next.insert(std::move(candidate));
            }
        }
...
bool has_good_gamma_bareiss(State& state, const Matrix& matrix, Int det_matrix, const std::vector<Vec>& gamma_prefixes, int emax) {
    int size = static_cast<int>(matrix.size());
...
bool has_good_gamma_adjugate(State& state, const Matrix& matrix, Int det_matrix, const std::vector<Vec>& gamma_prefixes, int emax) {
    int size = static_cast<int>(matrix.size());
    int remaining = state.args.order - size;
    if (remaining <= 0) return true;
    Int multiplier = state.pow_order_minus_one[state.args.order - size - 1];
    int n_minus_c = state.args.order - 1;
    std::vector<int> values = allowed_values(state.phi, emax);
    IntMatrix adjugate = adjugate_by_cofactors(matrix);
    ...
    for (const Vec& prefix : gamma_prefixes) {
        for (int e1 : values) {
            for (int e2 : values) {
                ...
                gamma.push_back(e2);
                state.gamma_tests += 1;
                Int d = det_augmented_from_adjugate(gamma, det_matrix, adjugate, 1);
                Int positive_d = d > 0 ? d : 0;
                Int upper_bound = multiplier * (static_cast<Int>(n_minus_c) * det_matrix + static_cast<Int>(remaining) * positive_d);
                if (upper_bound >= state.threshold_det) return true;
            }
...
bool has_good_gamma(State& state, const Matrix& matrix, Int det_matrix, const std::vector<Vec>& gamma_prefixes, int emax) {
    if (state.args.km_evaluator == "bareiss") return has_good_gamma_bareiss(state, matrix, det_matrix, gamma_prefixes, emax);
    if (state.args.km_evaluator == "adjugate") return has_good_gamma_adjugate(state, matrix, det_matrix, gamma_prefixes, emax);
    std::cerr << "Unknown KM evaluator: " << state.args.km_evaluator << "\n";
    std::exit(2);
}
...
void write_progress(const State& state, int current_depth) {
    ...
    out << "  \"km_evaluator\": \"" << state.args.km_evaluator << "\",\n";
...
void dfs(
    ...
    const IntMatrix& matrix_adjugate,
    const std::vector<std::vector<Vec>>& f_lists,
    int emax
) {
    ...
    for (const Vec& vector : candidate_vectors) {
        ...
        Int determinant = det_extension_from_adjugate(vector, matrix_determinant, matrix_adjugate, state.args.order);
        if (determinant <= 0) {
        ...
        if (next_size == state.args.order) {
            ...
            IntMatrix child_adjugate = adjugate_by_cofactors(candidate);
            dfs(state, candidate, determinant, child_adjugate, child_lists, child_emax);
        }
    }
}
...
Args parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (key == "--km-evaluator") args.km_evaluator = require_value(key);
...
int main(int argc, char** argv) {
    ...
    Int initial_determinant = state.args.order;
    IntMatrix initial_adjugate = {{1}};
    std::vector<std::vector<Vec>> f_lists(state.args.order + 1);
    f_lists[0].push_back(Vec{});
    int initial_emax = state.args.order - 2;

    dfs(state, initial, initial_determinant, initial_adjugate, f_lists, initial_emax);
