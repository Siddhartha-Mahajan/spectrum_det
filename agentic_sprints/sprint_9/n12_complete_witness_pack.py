#!/usr/bin/env python3
"""Complete the conjectured n=12 witness pack."""
...
def load_witnesses(path: Path) -> dict[int, Matrix]:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    seed_path = sprint_dir / "outputs" / "n12_low_interval_witnesses.json"
    witnesses = load_witnesses(seed_path)
    seed_count = len(witnesses)
    ...
    found_targets = sorted(value for value in targets if value in witnesses)
    missing_targets = sorted(value for value in targets if value not in witnesses)
    ...
    (output_dir / "n12_complete_witnesses.json").write_text(json.dumps(witness_payload, indent=2) + "\n")
    ...
    result = {
        ...
        "seed_witness_count": seed_count,
        "total_witness_count": len(witnesses),
        "found_conjectured_count": len(found_targets),
        "missing_conjectured_count": len(missing_targets),
        "missing_conjectured_values": missing_targets,
        "missing_conjectured_ranges": compress_ranges(missing_targets),
        ...
        "found_conjectured_ranges": compress_ranges(found_targets),
    ...
    (output_dir / "n12_complete_witness_results.json").write_text(json.dumps(result, indent=2) + "\n")
    return result
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min-target", type=int, default=301)
    parser.add_argument("--max-target", type=int, default=738)
    parser.add_argument("--random-samples", type=int, default=200000)
    parser.add_argument("--target-restarts", type=int, default=25)
    parser.add_argument("--target-steps", type=int, default=140)
...
def main() -> None:
    ...
    print(json.dumps({
        ...
        "found_conjectured_count": result["found_conjectured_count"],
        "missing_conjectured_count": result["missing_conjectured_count"],
